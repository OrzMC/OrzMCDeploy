...
check "工作区 env 指向宿主实例路径" "1" "$(printf '%s' "$DOCKER_RUN_LOGLINE" | grep -c 'MCSM_DOCKER_WORKSPACE_PATH=E:/orzmc/instances')"
check "自挂载 data 卷 target" "1" "$(printf '%s' "$DOCKER_RUN_LOGLINE" | grep -c 'target=/opt/mcsmanager/daemon/data')"
check "自挂载 instances target 含 DATA_ROOT" "1" "$(printf '%s' "$DOCKER_RUN_LOGLINE" | grep -c "target=/opt/mcsmanager/daemon/${DATA_ROOT}/instances")"
